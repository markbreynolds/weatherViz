...
#include "shaders.hpp" // src/assets/shaders.hpp
...
void Object::draw(GLuint shader, glm::mat4 view, glm::mat4 projection) {
  ...
  else if (material->getShaderType() == SkyShader) {
    GLuint textureID = glGetUniformLocation(shader, SKY_DIFFUSE_SAMPLER_UNIFORM_LOCATION);
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, material->getDiffuseTexture());
    glUniform1i(textureID, 0);
  }
